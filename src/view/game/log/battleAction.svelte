<script lang="ts">
	...
	import { getText, getTextTalent, join, withIndex } from 'src/misc/misc';
	import type { LogBattleAction } from 'src/models/log/LogBattleAction';
	...
	export let entry: LogBattleAction;
	...
	$: tacticsInformation = entry.tacticsInformation;
...
<div id="log-{index}" class="log first">
	...
	Kampf mit {getText(entry.weapon?.Name)}
	{getTextTalent(entry.talent, charState.char)}
	...
	<table>
		{#each $tacticsInformation as tactic}
			<tr>
				<td>
					{getText(tactic.tactic.Name)}
					{Object.keys(tactic.variables).length > 0 ? join(Object.entries(tactic.variables).map(([key,value])=>`${key}: ${value}`),', ') : ''}
				</td>
				<td>
					{#each tactic.support as support}
						{getText(support.tactic.Name)}
						{Object.keys(support.variables).length > 0 ? join(Object.entries(support.variables).map(([key,value])=>`${key}: ${value}`),', ') : ''}
					{/each}
					...
					<li>
						...
						{:else if mod.type == 'substitute'}
							{#if mod.roleIndex == 'any'}
								...
								{/if}
							{:else}
								{mod.text} ersetzt {getText(entry.roles[mod.roleIndex].name, char)} mit {entry
									.roles[mod.roleIndex].role} ≤ {entry.roles[mod.roleIndex].target}.
								{#if !$modifier.some((x) => x.id == mod.id)}
									(Nicht verwendet)
								{/if}
