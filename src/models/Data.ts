...
export class Data {
    ...
    public readonly TaktikMap: Record<string, Readonly<_Taktik>>;
    ...
    constructor(data: Daten, digest: string) {
        ...
        this.TaktikMap = data.Daten.Taktiken.Taktik.reduce((p, c) => { p[c.Id] = c; return p; }, {} as Record<string, _Taktik>)
    ...
    public static async fromJson(notaData: Daten) {
        ...
        const data = notaData ? new Data(notaData, digest) : undefined;
        return data;
    }
    /**
     * init
    ...
    public static async init(local: boolean, id?: string) {
        ...
        async function download(local: boolean) {
            ...
            const notaData = await parser.parse(data, async path => {
                const uri = `https://nota-game.github.io/Content/vNext/data/${path}`;
